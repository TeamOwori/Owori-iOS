...
struct StoryDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                ImageTabView(images: storyInfo.images_id ?? [], currentIndex: $currentIndex)
                    .frame(width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height * 0.4)
                    .clipped()
                
                VStack(spacing: 0) {
                    HorizontalImageScrollView(images: storyInfo.images_id ?? [], currentIndex: $currentIndex)
