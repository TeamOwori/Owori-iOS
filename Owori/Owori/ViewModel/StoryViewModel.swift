...
fileprivate enum OworiAPI {
    ...
    enum Path: String {
        ...
        case storiesSearchTemp = "/api/v1/stories/search/temp"
    }
}
...
//                    self?.storyModel.stories[index].is_liked = jsonDictionary["is_liked"] as! Bool
    ...
    func searchStorySortByKeyword(user: User, keyword: String, completion: @escaping () -> Void) {
        ...
        urlComponents.scheme = OworiAPI.scheme
        urlComponents.host = OworiAPI.host
        urlComponents.path = OworiAPI.Path.storiesSearchTemp.rawValue
        urlComponents.queryItems = [
        ...
        // url 테스트 log
        print("[searchStorySortByKeyword url Log] : \(url)")
