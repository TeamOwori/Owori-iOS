...
struct StoryDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                StoryDetailImageSection(storyInfo: $storyInfo, currentIndex: $currentIndex)
                Spacer()
                    .frame(height: 20)
                StoryDetailContentSection(isFavorite: $isFavorite, storyInfo: $storyInfo, stories: $stories, storiesForCollection: $storiesForCollection, storyDetailViewIsActive: $storyDetailViewIsActive, storyDetailViewIsActiveFromStoryAlbum: $storyDetailViewIsActiveFromStoryAlbum)
            }
        }
